...
class Official_TLE_Spec_Real extends FunSuite
  with BeforeAndAfterAll
  with ShouldMatchers {

  // SGP4 precissions
//  needed for 0 min  
//  implicit val doubleEquality = TolerantNumerics.tolerantRealEquality(0.005)
//  needed for 360 min  
//  implicit val doubleEquality = TolerantNumerics.tolerantRealEquality(0.007)
//  needed for 1080 min  
//  implicit val doubleEquality = TolerantNumerics.tolerantRealEquality(0.01)
//  needed for 1440 min  
//    implicit val doubleEquality = TolerantNumerics.tolerantRealEquality(0.012)
 
  val expectedSGP4 : Map[Int, (Vector[Real],Vector[Real])] = 
...
//  def check(result: (Vector[Real],Vector[Real]), expected: (Vector[Real],Vector[Real]))
//     (implicit ev: org.scalautils.Equality[Real]) {
...
//  }
//    Note: using reals, the test passes but takes half an hour
